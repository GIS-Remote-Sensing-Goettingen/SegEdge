...
class Dinov3SatDenseExtractor(torch.nn.Module):
    ...
    def __init__(self, model_name="facebook/dinov3-vitl16-pretrain-sat493m", device="cpu"):
        super().__init__()
        offline = os.environ.get("TRANSFORMERS_OFFLINE", "").lower() in {"1", "true", "yes"}
        try:
            ...
            if offline:
                raise RuntimeError(
                    "TRANSFORMERS_OFFLINE is set but weights are missing. "
                    "Unset the variable or download the model beforehand."
                ) from exc
            warnings.warn(f"Falling back to network load for processor: {exc}")
        ...
        try:
            ...
            warnings.warn(f"Falling back to network load for model: {exc}")
            self.model = AutoModel.from_pretrained(
                model_name,
                torch_dtype=torch.float16 if use_fp16 else torch.float32,
                trust_remote_code=True,
